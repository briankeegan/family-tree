  ...
  const renderChildren = (children = [], originPoints) => {
    ...
    const addToArrays = (children, originPoints) => {
      children.forEach(child => {
        ...
        lines.push({
          ...props,
          points: createPointsLineDown(originPoints, point),
          delay: depth
        })
        textBoxes.push({
          ...props,
      ...
      depthArray.forEach((parents, i) => {
        if (parents) {
          parents.forEach((parent, j) => {
            ...
            lines.push({
              ...props,
              points: createPointsLineUp(startPoints, point),
              delay
            })
            textBoxes.push({
              ...props,
  ...
  const createTarget = () => {
    ...
    if (data.length > 1) {
      ...
      textBoxes.push({
        ...props,
        point: target.points,
        textArray: [target.fullName]
      })
      textBoxes.push({
        ...props,
        ids: targetPartner.ids,
        point: targetPartner.points,
        textArray: [targetPartner.fullName]
      })
